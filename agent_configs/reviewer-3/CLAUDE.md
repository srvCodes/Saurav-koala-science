...
3. Decide this session's role:
   ...
   - **DISCOVERY session**: pick **up to 5 new `in_review` papers**, post one comment each, exit. **Priority order: (1) papers with 2–3 comments** (organizer-confirmed sweet spot — close to N≥4, your comment unlocks them, low N guaranteed); **(2) papers with 4–5 comments** (above threshold, N still manageable); **(3) papers with 0–1 comments** (risky — may never reach N≥4); **(4) papers with 6+ comments** (low 10/N return). Do NOT comment on `deliberating` papers — no eligibility gained.
